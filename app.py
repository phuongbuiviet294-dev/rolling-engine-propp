...
LOCK_ROUND_START = 168 LOCK_ROUND_END = 180

MODES = [ {"name": "4v3", "top_windows": 4, "vote_required": 3, "window_min": 6, "window_max": 22}, {"name": "6v4", "top_windows": 6, "vote_required": 4, "window_min": 6, "window_max": 22}, {"name": "8v5", "top_windows": 8, "vote_required": 5, "window_min": 6, "window_max": 22}, ]

GAP = 1

WIN_GROUP = 2.5 LOSS_GROUP = -1.0

WIN_COLOR = 1.5 LOSS_COLOR = -1.0

PHASE_STOP_WIN = 3.5 PHASE_STOP_LOSS = -2.0

SESSION_STOP_WIN = 6.0 SESSION_STOP_LOSS = -2.0

GROUP_SESSION_STOP_WIN = 8.0 GROUP_SESSION_STOP_LOSS = -8.0

RULE MOI

Không xét tổng profit.

Chỉ READY nếu ván TRADE gần nhất có pnl_group > 0.

Nếu ván TRADE gần nhất pnl_group <= 0 hoặc chưa có trade nào thì WAIT.

PROFIT_LAST_TRADE_FILTER = True

KEEP_AFTER_LOSS_ROUNDS = 1

MIN_TRADES_PER_WINDOW = 16 RECENT_WINDOW_SIZE = 26 MIN_WINDOW_SPACING = 5 MAX_CANDIDATE_WINDOWS = 10

VALIDATE_LEN = 24 MIN_TRAIN_LEN = 120 MIN_VALIDATE_TRADES = 2 VALIDATE_MIN_DRAWDOWN = -6.0

RELOCK_SCAN_LEN = 6 RELOCK_BUFFER = 0

REPLAY_FROM = 180 SHOW_DEBUG_TABLES = False SHOW_STYLED_HISTORY = False SHOW_HISTORY_ROWS = 40

ENABLE_DOUBLE_BET_COLOR = True REQUIRE_COLOR_CONFIRM = False

================= TELEGRAM =================

DEFAULT_BOT_TOKEN = "" DEFAULT_CHAT_ID = "6655585286"

BOT_TOKEN = st.secrets["BOT_TOKEN"] if "BOT_TOKEN" in st.secrets else DEFAULT_BOT_TOKEN CHAT_ID = st.secrets["CHAT_ID"] if "CHAT_ID" in st.secrets else DEFAULT_CHAT_ID

TELEGRAM_SEND_MODE = "READY_ONLY" SENT_FILE = "/tmp/telegram_sent_rounds_last_trade_profit_filter.json"

def telegram_enabled(): return bool(BOT_TOKEN and CHAT_ID)

def send_telegram(msg): if not telegram_enabled(): return False try: url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage" r = requests.post(url, json={"chat_id": CHAT_ID, "text": msg}, timeout=5) return r.ok except Exception: return False

def send_signal_once(signal_name, current_round, msg): if TELEGRAM_SEND_MODE == "READY_ONLY" and signal_name != "READY": return False
...
@st.cache_data(ttl=30, show_spinner=False) def load_numbers(): url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=csv&cache={time.time()}" df = pd.read_csv(url) df.columns = [str(c).strip().lower() for c in df.columns]
...
def group_of(n): if n <= 3: return 1 if n <= 6: return 2 if n <= 9: return 3 return 4

def color_of_number(n): if n <= 4: return 1 if n <= 8: return 2 return 3

def color_text(c): if c == 1: return "RED" if c == 2: return "GREEN" if c == 3: return "BLUE" return "-"

def color_icon(c): if c == 1: return "🔴 RED" if c == 2: return "🟢 GREEN" if c == 3: return "🔵 BLUE" return "-"

numbers = load_numbers() groups = [group_of(n) for n in numbers] colors = [color_of_number(n) for n in numbers]

if len(groups) < LOCK_ROUND_START: st.error(f"Chưa đủ dữ liệu. Hiện có {len(groups)} rounds, cần ít nhất {LOCK_ROUND_START}.") st.stop()

================= HELPERS =================

def last_trade_profit_filter_from_hist(hist): """ Dùng cho NEXT STATUS: Chỉ TRUE nếu trade gần nhất có pnl_group > 0. Nếu chưa có trade nào thì FALSE. """ if not PROFIT_LAST_TRADE_FILTER: return True, None
...
last_trade_row = trade_rows.iloc[-1]
last_pnl = float(last_trade_row.get("pnl_group", 0.0))
return last_pnl > 0, last_pnl
...
def compute_max_drawdown(results, win_value, loss_value): if not results: return 0.0
...
def compute_recent_profit(results, recent_n, win_value, loss_value): if not results: return 0.0 tail = results[-recent_n:] return float(sum(win_value if r == 1 else loss_value for r in tail))

def compute_streak_metrics(results): if not results: return { "max_hit_streak": 0, "max_loss_streak": 0, "count_hit_streak_ge2": 0, "count_loss_streak_ge2": 0, "streak_score": -999999.0, }
...
def pick_spaced_windows(df_sorted, top_n, min_spacing): selected = [] for _, row in df_sorted.iterrows(): w = int(row["window"]) if all(abs(w - int(x["window"])) >= min_spacing for x in selected): selected.append(row.to_dict()) if len(selected) >= top_n: break return pd.DataFrame(selected)
...
def evaluate_window_group(seq_groups, w): profit = 0.0 trades = 0 wins = 0 results = []
...
def build_window_tables(train_groups, window_min, window_max): rows = [evaluate_window_group(train_groups, w) for w in range(window_min, window_max + 1)] df = pd.DataFrame(rows)
...
def find_best_auto_mode_in_range(all_groups, scan_start, scan_end): effective_scan_end = min(scan_end, len(all_groups) - 1)
...
for i in range(start_replay, len(groups)):
    ...
    last_trade_win_filter = (
        True
        if not PROFIT_LAST_TRADE_FILTER
        else (last_trade_pnl_group is not None and last_trade_pnl_group > 0)
    )

    can_trade_group = (
        final_signal
        and distance >= GAP
        and last_trade_win_filter
    )
    ...
    if trade and used_keep:
        ...
        state = "TRADE"
    elif final_signal and not last_trade_win_filter:
        state = "WAIT_LAST_TRADE_GROUP_NOT_WIN"
    elif new_signal:
        state = "SIGNAL"
    elif used_keep:
        state = "KEEP_WAIT"
    ...
    if trade:
        ...
        # ✅ cập nhật pnl group của ván trade gần nhất
    ...
    history_rows.append(
        {
            ...
            "used_keep": used_keep,
            "last_trade_win_filter": last_trade_win_filter,
...
@st.cache_data(ttl=20, show_spinner=False) def cached_simulate_engine(numbers_tuple): nums = list(numbers_tuple) grps = [group_of(n) for n in nums] cols = [color_of_number(n) for n in nums] return simulate_engine(nums, grps, cols)

================= RUN ENGINE =================

sim = cached_simulate_engine(tuple(numbers))

hist = sim["hist"] phase_profit_group = sim["phase_profit_group"] phase_profit_color = sim["phase_profit_color"] phase_profit_total = sim["phase_profit_total"] phase_hits_group = sim["phase_hits_group"] phase_hits_color = sim["phase_hits_color"]

total_profit_group = sim["total_profit_group"] total_profit_color = sim["total_profit_color"] total_profit_all_phase = sim["total_profit_all_phase"] total_hits_group = sim["total_hits_group"] total_hits_color = sim["total_hits_color"]

locked_windows = sim["locked_windows"] selected_lock_round = sim["selected_lock_round"] selected_mode = sim["selected_mode"] lock_mode = sim["lock_mode"] scan_df_all = sim["scan_df_all"] scan_df_filtered = sim["scan_df_filtered"] round_eval_df = sim["round_eval_df"] lock_scan_start = sim["lock_scan_start"] lock_scan_end = sim["lock_scan_end"] keep_bet_group = sim["keep_bet_group"] keep_rounds_left = sim["keep_rounds_left"] last_trade_was_loss = sim["last_trade_was_loss"] consecutive_losses = sim["consecutive_losses"] phase_loss_streak = sim["phase_loss_streak"] last_trade = sim["last_trade"] phase_index = sim["phase_index"] session_stop = sim["session_stop"] session_stop_reason = sim["session_stop_reason"] relock_count = sim["relock_count"] last_relock_trigger_round = sim["last_relock_trigger_round"] phase_summary_df = sim["phase_summary_df"] last_trade_pnl_group = sim["last_trade_pnl_group"]
...
if locked_windows and selected_mode is not None: vote_required = selected_mode["vote_required"]
...
next_round = len(groups) current_round = len(numbers)

preds_group = [groups[next_round - w] for w in locked_windows if next_round - w >= 0] preds_color = [colors[next_round - w] for w in locked_windows if next_round - w >= 0]

if preds_group and selected_mode is not None: vote_group, confidence_group = Counter(preds_group).most_common(1)[0] vote_required = selected_mode["vote_required"] else: vote_group, confidence_group = None, 0 vote_required = 0

if preds_color: vote_color, confidence_color = Counter(preds_color).most_common(1)[0] else: vote_color, confidence_color = None, 0

current_number = numbers[-1] if numbers else None current_group = groups[-1] if groups else None current_color = colors[-1] if colors else None

if not hist.empty: last_trade_rows = hist[hist["trade"] == True] distance = next_round - last_trade_rows["round"].max() if len(last_trade_rows) > 0 else 999 else: distance = 999

new_signal = confidence_group >= vote_required if vote_group is not None else False color_signal = confidence_color >= vote_required if vote_color is not None else False

used_keep_next = False final_vote_group = vote_group final_vote_color = vote_color

last_trade_win_filter, last_trade_pnl_group_next = last_trade_profit_filter_from_hist(hist)
...
if can_bet:
    next_state = "READY"
elif signal and not last_trade_win_filter:
    next_state = "WAIT_LAST_TRADE_GROUP_NOT_WIN"
else:
    next_state = "WAIT"

next_row = { "phase": phase_index, "round": next_round, "number": current_number, "group": current_group, "color": color_text(current_color), "mode": selected_mode["name"] if selected_mode else "-", "vote_required": selected_mode["vote_required"] if selected_mode else 0, "top_windows": selected_mode["top_windows"] if selected_mode else 0, "vote_group": vote_group, "confidence_group": confidence_group, "vote_color": color_text(vote_color), "confidence_color": confidence_color, "new_signal": new_signal, "color_signal": color_signal, "used_keep": used_keep_next, "last_trade_win_filter": last_trade_win_filter, "last_trade_pnl_group": last_trade_pnl_group_next, "keep_group": next_keep_bet_group, "keep_left": next_keep_rounds_left, "final_vote_group": final_vote_group, "final_vote_color": color_text(final_vote_color), "signal": signal, "trade": False, "bet_group": final_vote_group if can_bet else None, "bet_color": color_text(final_vote_color) if can_bet and ENABLE_DOUBLE_BET_COLOR else "-", "hit_group": None, "hit_color": None, "pnl_group": 0.0, "pnl_color": 0.0, "pnl_total": 0.0, "state": next_state, "phase_profit_group": phase_profit_group, "phase_profit_color": phase_profit_color, "phase_profit_total": phase_profit_total, "total_profit_group": total_profit_group, "total_profit_color": total_profit_color, "total_profit_all_phase": total_profit_all_phase, "consecutive_losses": consecutive_losses, "phase_loss_streak": phase_loss_streak, "locked_windows": ", ".join(map(str, locked_windows)), "relock_count": relock_count, "relock_triggered_now": False, }

hist_display = pd.concat([hist, pd.DataFrame([next_row])], ignore_index=True)

================= TELEGRAM NOTIFY =================

if telegram_enabled() and can_bet and final_vote_group is not None: ready_msg = ( f"READY DOUBLE BET\n" f"Round: {current_round}\n" f"Current Number: {current_number}\n" f"Current Group: {current_group}\n" f"Current Color: {color_icon(current_color)}\n" f"Bet Group: {final_vote_group}\n" f"Bet Color: {color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else 'OFF'}\n" f"Mode: {selected_mode['name'] if selected_mode else '-'}\n" f"Vote Group Strength: {confidence_group}\n" f"Vote Color Strength: {confidence_color}\n" f"Last Trade Group PNL: {last_trade_pnl_group_next}\n" f"Last Trade Win Filter: {last_trade_win_filter}\n" f"Total Profit Group: {total_profit_group}\n" f"Total Profit All: {total_profit_all_phase}\n" f"Stop Reason: {session_stop_reason}" )
...
st.title("🎯 Auto Relock Engine | Last Trade Group Win Filter")

c1, c2, c3, c4 = st.columns(4) c1.metric("Current Number", current_number if current_number is not None else "-") c2.metric("Current Group", current_group if current_group is not None else "-") c3.metric("Current Color", color_icon(current_color)) c4.metric("Next Group", final_vote_group if final_vote_group is not None else "-")

st.write("Next Color:", color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else "OFF") st.write("Selected Mode:", selected_mode["name"] if selected_mode else "-") st.write("Vote Required:", selected_mode["vote_required"] if selected_mode else 0) st.write("Top Windows:", selected_mode["top_windows"] if selected_mode else 0) st.write("Window Range:", f'{selected_mode["window_min"]}-{selected_mode["window_max"]}' if selected_mode else "-") st.write("Group Vote Strength:", confidence_group) st.write("Color Vote Strength:", confidence_color) st.write("Signal:", signal) st.write("Can Bet:", can_bet) st.write("State:", next_state) st.write("Last Trade Win Filter:", last_trade_win_filter) st.write("Last Trade Group PNL:", last_trade_pnl_group_next) st.write("Double Bet Color:", ENABLE_DOUBLE_BET_COLOR) st.write("Require Color Confirm:", REQUIRE_COLOR_CONFIRM) st.write("Group Stop Win:", GROUP_SESSION_STOP_WIN) st.write("Group Stop Loss:", GROUP_SESSION_STOP_LOSS) st.write("Best Lock Round:", selected_lock_round) st.write("Scan Range:", f"{lock_scan_start} -> {lock_scan_end}") st.write("Lock Mode:", lock_mode) st.write("Relock Count:", relock_count) st.write("Last Relock Trigger Round:", last_relock_trigger_round) st.write("Session Stop:", session_stop) st.write("Session Stop Reason:", session_stop_reason) st.write("Telegram Enabled:", telegram_enabled())

if lock_mode == "fallback": st.warning("Đang dùng bộ lock fallback.")

if session_stop: if session_stop_reason in ("SESSION_STOP_WIN", "GROUP_STOP_WIN"): st.success(f"✅ {session_stop_reason}") elif session_stop_reason in ("SESSION_STOP_LOSS", "GROUP_STOP_LOSS"): st.error(f"⛔ {session_stop_reason}") elif can_bet and final_vote_group is not None: st.markdown( f""" <div style="background:#ff4b4b;padding:22px;border-radius:10px;text-align:center;font-size:28px;color:white;font-weight:bold;"> READY DOUBLE BET<br> GROUP {final_vote_group} | COLOR {color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else "OFF"}<br> LAST TRADE GROUP PNL → {last_trade_pnl_group_next}<br> MODE → {selected_mode["name"] if selected_mode else "-"} </div> """, unsafe_allow_html=True, ) else: if not last_trade_win_filter: st.info("WAIT | LAST TRADE GROUP PNL <= 0 OR NO PREVIOUS TRADE") else: st.info("WAIT")

st.subheader("Current Phase Stats") s1, s2, s3, s4 = st.columns(4) s1.metric("Phase Profit Group", phase_profit_group) s2.metric("Phase Profit Color", phase_profit_color) s3.metric("Phase Profit Total", phase_profit_total) s4.metric("Phase Trades", len(phase_hits_group))

s5, s6, s7 = st.columns(3) s5.metric("Phase WR Group %", round(np.mean(phase_hits_group) * 100, 2) if phase_hits_group else 0) s6.metric("Phase WR Color %", round(np.mean(phase_hits_color) * 100, 2) if phase_hits_color else 0) s7.metric("Phase Loss Streak", phase_loss_streak)

st.subheader("Session Stats") t1, t2, t3, t4 = st.columns(4) t1.metric("Total Profit Group", total_profit_group) t2.metric("Total Profit Color", total_profit_color) t3.metric("Total Profit All", total_profit_all_phase) t4.metric("Last Trade Round", last_trade if last_trade is not None else "-")

t5, t6, t7 = st.columns(3) t5.metric("Total Trades", len(total_hits_group)) t6.metric("Total WR Group %", round(np.mean(total_hits_group) * 100, 2) if total_hits_group else 0) t7.metric("Total WR Color %", round(np.mean(total_hits_color) * 100, 2) if total_hits_color else 0)

st.subheader("Current Lock Backtest Check - Group Only") b1, b2, b3, b4 = st.columns(4) b1.metric("Scan Trades", scan_range_bt["trades"]) b2.metric("Scan Profit Group", scan_range_bt["profit_group"]) b3.metric("Scan Winrate Group %", round(scan_range_bt["winrate_group"] * 100, 2)) b4.metric("Scan MaxDD Group", scan_range_bt["max_drawdown_group"])

d1, d2, d3, d4 = st.columns(4) d1.metric("Post-lock Trades", post_lock_bt["trades"]) d2.metric("Post-lock Profit Group", post_lock_bt["profit_group"]) d3.metric("Post-lock Winrate Group %", round(post_lock_bt["winrate_group"] * 100, 2)) d4.metric("Post-lock MaxDD Group", post_lock_bt["max_drawdown_group"])

st.subheader("Phase Profit Total Curve") if not hist_display.empty: current_phase_df = hist_display[hist_display["phase"] == phase_index].copy() if not current_phase_df.empty: st.line_chart(current_phase_df["phase_profit_total"].reset_index(drop=True))

st.subheader("Total Profit Curve") if not hist_display.empty: st.line_chart(hist_display["total_profit_all_phase"].reset_index(drop=True))

with st.expander("Phase Summary"): st.dataframe(phase_summary_df, use_container_width=True)

if SHOW_DEBUG_TABLES: with st.expander("Round Evaluation"): st.dataframe(round_eval_df, use_container_width=True)
...
st.subheader("History") history_view = hist_display.iloc[::-1].head(SHOW_HISTORY_ROWS).copy()

if SHOW_STYLED_HISTORY: def highlight_trade(row): if row["state"] in ("READY",): return ["background-color: #ffd700"] * len(row) if row["state"] == "TRADE_KEEP": return ["background-color: #ffb347; color:black"] * len(row) if row["state"] in ("WAIT_LAST_TRADE_GROUP_NOT_WIN",): return ["background-color: #555555; color:white"] * len(row) if row["state"] in ("AUTO_RELOCK_LOSS", "AUTO_RELOCK_WIN"): return ["background-color: #32cd32; color:black"] * len(row) if row["state"] in ("SESSION_STOP_WIN", "GROUP_STOP_WIN"): return ["background-color: #2e8b57; color:white"] * len(row) if row["state"] in ("SESSION_STOP_LOSS", "GROUP_STOP_LOSS"): return ["background-color: #d9534f; color:white"] * len(row) if row["trade"]: return ["background-color: #ff4b4b; color:white"] * len(row) return [""] * len(row)
...
else: st.dataframe(history_view, use_container_width=True)
