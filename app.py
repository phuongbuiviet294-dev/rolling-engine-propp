...
# chỉ giữ 3 mode
MODES = [
    {"name": "4v3", "top_windows": 5, "vote_required": 3, "window_min": 6, "window_max": 22},
        ...
        if trade:
            ...
            if relock_triggered_now:
                ...
                if new_selected_lock_round is not None and new_selected_mode is not None:
                    relock_count += 1
                    last_relock_trigger_round = current_round
...
if session_stop:
    ...
    st.markdown(
        ...
        BET GROUP → {final_vote_group} | MODE → {selected_mode["name"] if selected_mode else "-"}
        </div>
        """,
