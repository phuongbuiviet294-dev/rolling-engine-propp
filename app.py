...
# ✅ RULE MỚI
# Không bet lệnh đầu.
# Chỉ READY nếu total_profit_group trước đó > 0.
PROFIT_POSITIVE_FILTER = True
ALLOW_FIRST_TRADE = False
...
SENT_FILE = "/tmp/telegram_sent_rounds_profit_group_filter.json"
...
# ================= HELPERS =================
def profit_filter_ok(total_profit_group, total_hits_group):
    if not PROFIT_POSITIVE_FILTER:
        return True

    # Không cho lệnh đầu tiên
    if len(total_hits_group) == 0:
        return False

    # Chỉ cho READY khi profit group trước đó đang dương
    return total_profit_group > 0
...
def simulate_engine(numbers, groups, colors):
    ...
    for i in range(start_replay, len(groups)):
        ...
        profit_group_filter = profit_filter_ok(total_profit_group, total_hits_group)
        ...
        if trade and used_keep:
            ...
            state = "WAIT_PROFIT_GROUP_NOT_POSITIVE"
        elif new_signal:
            state = "SIGNAL"
        elif used_keep:
            state = "KEEP_WAIT"
...
if session_stop:
    ...
    if can_bet:
        ...
        next_state = "WAIT_PROFIT_GROUP_NOT_POSITIVE"
    else:
        next_state = "WAIT"
...
if telegram_enabled() and can_bet and final_vote_group is not None:
    ready_msg = (
        ...
        f"Profit Group Filter: {profit_group_filter}\n"
...
# ================= UI =================
st.title("🎯 Auto Relock Engine | Profit Group Filter")
...
st.write("Profit Group Filter:", profit_group_filter)
...
if session_stop:
    ...
    st.markdown(
        ...
        TOTAL PROFIT GROUP → {total_profit_group}<br>
        MODE → {selected_mode["name"] if selected_mode else "-"}
        </div>
        """,
    ...
    if not profit_group_filter:
        st.info("WAIT | TOTAL PROFIT GROUP <= 0")
    else:
        st.info("WAIT")
...
if SHOW_STYLED_HISTORY:
    def highlight_trade(row):
        ...
        if row["state"] in ("WAIT_PROFIT_GROUP_NOT_POSITIVE",):
