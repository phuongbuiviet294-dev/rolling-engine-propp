...
st.set_page_config(page_title="LOCK75 CLEAN PATTERN ENGINE", layout="wide")
...
STATE_FILE = "/tmp/real_live_lock75_clean_state.json"
...
def calc_pattern_stats(groups, pattern, bet_label):
    ...
    stability = profit / max_drawdown if max_drawdown > 0 else (profit if profit > 0 else 0.0)
...
def score_pattern(stat, recent, trans):
    ...
    score = 0.0
    score += profit * 0.9
    score += recent_profit * 4.2
    score += stat["profit_per_trade"] * 8.0
    score += wr * 4.0
    score += recent_wr * 9.0
    score += math.log1p(trades) * 2.5

    score -= stat["max_loss_streak"] * 1.8
    score -= stat["current_loss_streak"] * 3.0
    score -= abs(min(stat["min_equity"], 0)) * 0.7
    score -= stat["max_drawdown"] * 0.6

    if trans["transition_ok"]:
        score += trans["transition_dominance"] * 12.0
        score += trans["transition_top_ratio"] * 4.0

    if stat["stability"] >= STABILITY_MIN:
        score += 2.5
...
def add_side_strength_fields(rows):
    ...
    for r in rows:
        grouped.setdefault((r["pattern"], r["len"], tuple(r["tail"])), []).append(r)

    clean_rows = []

    for _, items in grouped.items():
        total_trades = sum(max(x["trades"], 0) for x in items)
        ...
        for r in items:
            ...
            r["top1_ratio"] = r["trades"] / total_trades if total_trades else 0.0
            r["is_best_side"] = r is by_score[0]

        # Chỉ giữ 1 side tốt nhất cho mỗi tail để tránh ABBC -> A/B/C loạn
...
def choose_signal(groups):
    ...
    for m in matches:
        ...
        if m["profit"] < MIN_PROFIT:
            continue
        if m["profit_per_trade"] < PROFIT_PER_TRADE_MIN:
            continue
        ...
        if m["recent_check_count"] >= RECENT_CHECK_N and m["recent_hits"] < RECENT_HIT_MIN:
            continue
        if m["top1_ratio"] < TOP1_RATIO_MIN:
            continue
        if m["profit_gap"] < PROFIT_GAP_MIN:
            continue
        if m["stability"] < STABILITY_MIN:
            continue
        if m["broken"]:
            continue
    ...
    return None, "WAIT_PATTERN_RUBBISH_FILTERED", matches
...
def simulate_lock75_backtest(groups_tuple):
    ...
    for target_idx in range(LOCK_ROWS, len(groups)):
        ...
        rows.append({
            ...
            "profit_per_trade": round(sig["profit_per_trade"], 3) if sig else None,
            "stability": round(sig["stability"], 3) if sig else None,
        })
...
def discover_top_patterns(groups):
    ...
    for pattern in pattern_set:
        for bet_label in labels_in_pattern(pattern):
            ...
            rows.append({
                ...
                "profit_per_trade": round(stat["profit_per_trade"], 3),
                "recent_wr": round(recent["wr"] * 100, 2),
                "recent_profit": round(recent["profit"], 2),
                "recent_hits": stat["recent_hits"],
                "max_loss_streak": stat["max_loss_streak"],
                "max_drawdown": round(stat["max_drawdown"], 2),
    ...
    df = df.sort_values(
        ["profit_per_trade", "recent_profit", "profit", "wr"],
...
# UI
# =========================
st.title("LOCK75 CLEAN GROUP PATTERN ENGINE | RUBBISH FILTER")
...
with st.expander("CONFIG"):
    ...
    st.write("STABILITY_MIN:", STABILITY_MIN)
    st.write("TOP1_RATIO_MIN:", TOP1_RATIO_MIN)
    st.write("RECENT_MIN_PROFIT:", RECENT_MIN_PROFIT)
    st.write("RECENT_WR_MIN:", RECENT_WR_MIN)
    st.write("RECENT_HIT_MIN:", RECENT_HIT_MIN)
...
if signal:
    ...
    st.write("Profit/Trade:", round(signal["profit_per_trade"], 3))
    st.write("Recent Profit:", round(signal["recent_profit"], 2))
    st.write("Recent Hits:", f"{signal['recent_hits']}/{signal['recent_check_count']}")
    st.write("Stability:", round(signal["stability"], 3))
else:
...
if matches:
    ...
    for col in [
        ...
        if col in dfm.columns:
            dfm[col] = dfm[col].round(3)

    dfm["transition_dominance"] = dfm["transition_dominance"].round(3)
    dfm["transition_top_ratio"] = dfm["transition_top_ratio"].round(3)
