...
SESSION_STOP_LOSS = -2.0

GROUP_SESSION_STOP_WIN = 8.0
GROUP_SESSION_STOP_LOSS = -8.0


# ===== RULE FILTER =====
# KHÔNG bet lệnh đầu
# CHỈ bet nếu LỆNH TRƯỚC đó thắng (profit group > 0 ngay trước)
PROFIT_POSITIVE_FILTER = True
...
def send_signal_once(signal_name, current_round, msg):
    ...
    signal_key = f"{signal_name}|{current_round}"
...
if len(groups) < LOCK_ROUND_START:
    st.error("Chưa đủ data")
    st.stop()

# ================= HELPERS =================
def profit_filter_ok(last_trade_pnl_group):
    if not PROFIT_POSITIVE_FILTER:
        return True
...
def build_window_tables(train_groups, window_min, window_max):
    rows = [
        evaluate_window_group(train_groups, w)
...
def simulate_engine(numbers, groups, colors):
    ...
    # chỉ khi trade gần nhất thắng group thì trade sau mới được phép READY.
    last_trade_pnl_group = None
    ...
    for i in range(start_replay, len(groups)):
        ...
        vote_color, confidence_color = (
            Counter(preds_color).most_common(1)[0]
            if preds_color
            else (None, 0)
        )
        ...
        if new_signal:
            ...
            if (
                last_trade_was_loss
                and keep_rounds_left > 0
                and keep_bet_group is not None
            ):
        ...
        profit_group_filter = profit_filter_ok(last_trade_pnl_group)
        ...
        if trade:
            ...
            # Cập nhật ván trade gần nhất.
            # Ván sau chỉ được READY nếu giá trị này > 0.
...
if phase_profit_total <= PHASE_STOP_LOSS:
            ...
            if relock_triggered_now:
                phase_summary_rows.append(
                    {
                        ...
                        "phase_winrate_group": round(np.mean(phase_hits_group) * 100, 2)
                        if phase_hits_group
                        else 0.0,
                        "phase_winrate_color": round(np.mean(phase_hits_color) * 100, 2)
                        if phase_hits_color
                        else 0.0,
                ...
                if new_selected_lock_round is not None and new_selected_mode is not None:
                    ...
                    # Sau khi relock, reset điều kiện lệnh trước
                    # để tránh dùng kết quả phase cũ cho phase mới.
                    last_trade_pnl_group = None
...
if session_stop:
    ...
    if new_signal:
        ...
        if (
            last_trade_was_loss
            and next_keep_rounds_left > 0
            and next_keep_bet_group is not None
        ):
...
if telegram_enabled() and can_bet and final_vote_group is not None:
    ready_msg = (
        ...
        f"Previous Group PNL: {last_trade_pnl_group}\n"
...
# ================= UI =================
st.title("Auto Relock Engine | Previous Group Win Filter")
...
st.write("Previous Group PNL:", last_trade_pnl_group)
st.write("Profit Filter OK:", profit_group_filter)
st.write("Total Profit Group:", total_profit_group)
st.write("Total Profit All:", total_profit_all_phase)
...
if session_stop:
    ...
    st.markdown(
        ...
        PREVIOUS GROUP PNL → {last_trade_pnl_group}<br>
        MODE → {selected_mode["name"] if selected_mode else "-"}
        </div>
        """,
...
s5.metric(
    "Phase WR Group %",
    round(np.mean(phase_hits_group) * 100, 2) if phase_hits_group else 0,
)
s6.metric(
    "Phase WR Color %",
    round(np.mean(phase_hits_color) * 100, 2) if phase_hits_color else 0,
)
...
t6.metric(
    "Total WR Group %",
    round(np.mean(total_hits_group) * 100, 2) if total_hits_group else 0,
)
t7.metric(
    "Total WR Color %",
    round(np.mean(total_hits_color) * 100, 2) if total_hits_color else 0,
)
...
history_view = hist_display.iloc[::-1].head(40).copy()
