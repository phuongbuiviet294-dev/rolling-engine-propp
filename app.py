...
for i, n in enumerate(numbers):
    ...
    wr_live = np.mean(hits_all[-20:]) if len(hits_all) >= 20 else 0.3

    # ===== GENERATE SIGNAL =====
    if cooldown == 0 and len(engine) >= 40 and i - last_trade_round > BASE_GAP:
        best_window = None
        best_ev = -999
        best_wr = 0
        ...
        if wr_live < 0.27:
            allow_trade = best_ev > 0.08
        else:
            allow_trade = best_ev > 0
...
# ================= DASHBOARD =================
st.title("✅ STABLE CYCLE ENGINE — FINAL")
...
st.caption(f"WR_live={wr_live:.2f} | Cooldown={cooldown}")
