...
WIN_GROUP = 2.5 LOSS_GROUP = -1.0 WIN_COLOR = 1.5 LOSS_COLOR = -1.0

PHASE_STOP_WIN = 3.5 PHASE_STOP_LOSS = -2.0

SESSION_STOP_WIN = 200.0 SESSION_STOP_LOSS = -200.0

GROUP_SESSION_STOP_WIN = 15.0 GROUP_SESSION_STOP_LOSS = -80.0

KEEP_AFTER_LOSS_ROUNDS = 1

MIN_TRADES_PER_WINDOW = 16 RECENT_WINDOW_SIZE = 26 MIN_WINDOW_SPACING = 1 MAX_CANDIDATE_WINDOWS = 10
...
Window vote trước, pattern phải trùng group vote mới bet

ENABLE_PATTERN_FILTER = True STRICT_WINDOW_PATTERN_MATCH = True

Pattern ngắn A,A dễ nhiễu. Nếu muốn dùng thì True.
...
def compute_max_drawdown(results, win_value, loss_value): if not results: return 0.0 path = compute_profit_path(results, win_value, loss_value) peak = -10**18 max_dd = 0.0 for x in path: peak = max(peak, x) max_dd = min(max_dd, x - peak) return float(max_dd)
...
for i in range(start_replay, len(groups)):
    ...
    vote_signal = confidence_group >= vote_required
    ...
    pattern_match_runtime = pattern_group_runtime is not None and vote_signal and pattern_group_runtime == vote_group

    final_vote_group = vote_group if pattern_match_runtime else None
    final_vote_color = vote_color
    new_signal = pattern_match_runtime
    ...
    if trade and used_keep:
        state = "TRADE_KEEP"
    elif trade and pattern_match_runtime:
        state = "TRADE_WINDOW_PATTERN"
    elif trade:
        state = "TRADE"
    elif new_signal and pattern_match_runtime:
        state = "SIGNAL_WINDOW_PATTERN"
    elif new_signal:
        state = "SIGNAL"
    elif used_keep:
        state = "KEEP_WAIT"
    ...
    history_rows.append(
        {
            ...
            "pattern_match_vote": pattern_match_runtime,
...
@st.cache_data(ttl=20, show_spinner=False) def cached_simulate_engine_pattern_strict(numbers_tuple): nums = list(numbers_tuple) grps = [group_of(n) for n in nums] cols = [color_of_number(n) for n in nums] return simulate_engine(nums, grps, cols)

sim = cached_simulate_engine_pattern_strict(tuple(numbers))
...
vote_signal = confidence_group >= vote_required if vote_group is not None else False color_signal = confidence_color >= vote_required if vote_color is not None else False
...
st.write("Next Color:", color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else "OFF") st.write("Vote Group:", vote_group) st.write("Vote Signal:", vote_signal) st.write("Pattern Type:", pattern_type) st.write("Pattern Group:", pattern_group) st.write("Pattern Match Vote:", pattern_match_vote) st.write("Selected Mode:", selected_mode["name"] if selected_mode else "-") st.write("Vote Required:", selected_mode["vote_required"] if selected_mode else 0) st.write("Top Windows:", selected_mode["top_windows"] if selected_mode else 0) st.write("Window Range:", f'{selected_mode["window_min"]}-{selected_mode["window_max"]}' if selected_mode else "-") st.write("Group Vote Strength:", confidence_group) st.write("Color Vote Strength:", confidence_color) st.write("Double Bet Color:", ENABLE_DOUBLE_BET_COLOR) st.write("Require Color Confirm:", REQUIRE_COLOR_CONFIRM) st.write("Strict Window Pattern Match:", STRICT_WINDOW_PATTERN_MATCH) st.write("Expanded Pattern Filter:", ENABLE_PATTERN_FILTER) st.write("Group Stop Win:", GROUP_SESSION_STOP_WIN) st.write("Group Stop Loss:", GROUP_SESSION_STOP_LOSS) st.write("Best Lock Round:", selected_lock_round) st.write("Scan Range:", f"{lock_scan_start} -> {lock_scan_end}") st.write("Lock Mode:", lock_mode) st.write("Relock Count:", relock_count) st.write("Last Relock Trigger Round:", last_relock_trigger_round) st.write("Session Stop:", session_stop) st.write("Session Stop Reason:", session_stop_reason) st.write("Telegram Enabled:", telegram_enabled())

if lock_mode == "fallback": st.warning("Đang dùng bộ lock fallback.")

if session_stop: if session_stop_reason in ("SESSION_STOP_WIN", "GROUP_STOP_WIN"): st.success(f"✅ {session_stop_reason}") elif session_stop_reason in ("SESSION_STOP_LOSS", "GROUP_STOP_LOSS"): st.error(f"⛔ {session_stop_reason}") elif can_bet and final_vote_group is not None: st.markdown( f""" <div style="background:#ff4b4b;padding:22px;border-radius:10px;text-align:center;font-size:28px;color:white;font-weight:bold;"> READY WINDOW + PATTERN<br> GROUP {final_vote_group} | COLOR {color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else "OFF"}<br> VOTE {vote_group} = PATTERN {pattern_group}<br> PATTERN → {pattern_type}<br> MODE → {selected_mode["name"] if selected_mode else "-"} </div> """, unsafe_allow_html=True, ) else: st.info(f"WAIT | Vote={vote_group} | Pattern={pattern_type} -> {pattern_group} | Match={pattern_match_vote}")
...
if SHOW_STYLED_HISTORY: def highlight_trade(row): if row["state"] in ("READY",): return ["background-color: #ffd700"] * len(row) if row["state"] == "TRADE_KEEP": return ["background-color: #ffb347; color:black"] * len(row) if row["state"] in ("TRADE_WINDOW_PATTERN", "SIGNAL_WINDOW_PATTERN"): return ["background-color: #ff4b4b; color:white"] * len(row) if row["state"] in ("AUTO_RELOCK_LOSS", "AUTO_RELOCK_WIN"): return ["background-color: #32cd32; color:black"] * len(row) if row["state"] in ("SESSION_STOP_WIN", "GROUP_STOP_WIN"): return ["background-color: #2e8b57; color:white"] * len(row) if row["state"] in ("SESSION_STOP_LOSS", "GROUP_STOP_LOSS"): return ["background-color: #d9534f; color:white"] * len(row) if row["trade"]: return ["background-color: #ff4b4b; color:white"] * len(row) return [""] * len(row)
