...
# ================= TELEGRAM =================
DEFAULT_BOT_TOKEN = "PASTE_YOUR_BOT_TOKEN_HERE"
...
def telegram_enabled():
    return bool(BOT_TOKEN and CHAT_ID and BOT_TOKEN != "PASTE_YOUR_BOT_TOKEN_HERE")
...
def send_signal_once(signal_name: str, current_round: int, msg: str) -> bool:
    if not telegram_enabled():
        return False
...
def simulate_engine(numbers, groups, colors):
    ...
    for i in range(start_replay, len(g)):
        # SESSION STOP THEO GROUP PROFIT
        if total_profit_group >= SESSION_STOP_WIN:
            break
        if total_profit_group <= SESSION_STOP_LOSS:
            break

        preds_group = [g[i - w] for w in ws if i - w >= 0]
        preds_color = [c[i - w] for w in ws if i - w >= 0]
        ...
        if trade:
            ...
            # PHASE WIN / LOSS THEO GROUP PROFIT
            if phase_profit_group <= PHASE_STOP_LOSS:
                relock_triggered_now = True
                relock_reason_now = "PHASE_STOP_LOSS_GROUP"
                state = "AUTO_RELOCK_LOSS"
            elif phase_profit_group >= PHASE_STOP_WIN:
                relock_triggered_now = True
                relock_reason_now = "PHASE_TAKE_PROFIT_GROUP"
            ...
            if relock_triggered_now:
                ...
                (
                    ...
                    new_round_eval_df,
                    new_lock_mode,
                ) = find_best_auto_mode_in_range(g, scan_start, scan_end)
    ...
    return {
        ...
        "session_stop_reason": (
            "SESSION_STOP_WIN_GROUP"
            if total_profit_group >= SESSION_STOP_WIN
            else "SESSION_STOP_LOSS_GROUP"
            if total_profit_group <= SESSION_STOP_LOSS
            else None
        ),
        "relock_count": relock_count,
...
# ================= UI =================
st.title("🎯 Auto Relock Engine | Phase by GROUP Profit")
...
if session_stop:
    if session_stop_reason == "SESSION_STOP_WIN_GROUP":
        st.success("✅ SESSION STOP WIN GROUP")
    elif session_stop_reason == "SESSION_STOP_LOSS_GROUP":
        st.error("⛔ SESSION STOP LOSS GROUP")
    ...
    st.markdown(
        ...
        COLOR {color_icon(final_vote_color) if ENABLE_DOUBLE_BET_COLOR else "OFF"}<br>
        MODE → {selected_mode["name"] if selected_mode else "-"}
        </div>
        """,
...
st.subheader("Phase Profit Group Curve")
...
st.subheader("Total Profit Group Curve")
if not hist_display.empty:
    st.line_chart(hist_display["total_profit_group"].reset_index(drop=True))

st.subheader("Total Profit All Curve")
if not hist_display.empty:
...
if SHOW_STYLED_HISTORY:
    def highlight_trade(row):
        ...
        if row["state"] == "SESSION_STOP_WIN_GROUP":
            return ["background-color: #2e8b57; color:white"] * len(row)
        if row["state"] == "SESSION_STOP_LOSS_GROUP":
