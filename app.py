...
Khong dung total_profit_group de loc lenh.

Chi READY neu trade gan nhat co pnl_group > 0.

Neu trade gan nhat pnl_group <= 0 hoac chua co trade nao thi WAIT.
...
TELEGRAM_SEND_MODE = "READY_ONLY" SENT_FILE = "/tmp/telegram_sent_rounds_last_trade_pnl_filter.json"
...
if len(groups) < LOCK_ROUND_START: st.error(f"Chua du du lieu. Hien co {len(groups)} rounds, can it nhat {LOCK_ROUND_START}.") st.stop()

================= HELPERS =================

def last_trade_filter_from_hist(hist): if not PROFIT_LAST_TRADE_FILTER: return True, None
...
last_pnl_group = float(last_row.get("pnl_group", 0.0))
return last_pnl_group > 0, last_pnl_group
...
def compute_recent_profit(results, recent_n, win_value, loss_value): if not results: return 0.0
...
for i in range(start_replay, len(groups)):
    ...
    if trade and used_keep:
        ...
        state = "TRADE"
    elif final_signal and not last_trade_filter:
    ...
    if trade:
        ...
        # Cap nhat PNL cua trade group gan nhat de quyet dinh READY cho van sau.
    ...
    history_rows.append(
        {
            ...
            "used_keep": used_keep,
            "last_trade_filter": last_trade_filter,
...
last_trade_filter, last_trade_pnl_group_next = last_trade_filter_from_hist(hist)
...
st.title("Auto Relock Engine | Last Trade Group PNL Filter")
