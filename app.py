...
def get_run_signal(seq, min_run_len=2, max_run_len=5, min_samples=6, min_prob=0.40):
    if len(seq) < 5:
        return None
        ...
        if not bundle_df.empty:
            for _, row in bundle_df.iterrows():
                ...
                val1_bt = backtest_bundle_vote_range(
                    all_to_validate_groups,
                    all_to_validate_colors,
                    windows,
                    validate1_start,
                    validate1_end,
                )
...
for i in range(processed_until + 1, len(groups)):
    ...
    # run filter
    run_signal_group = get_run_signal(groups[:i], RUN_MIN_LEN, RUN_MAX_LEN, RUN_MIN_SAMPLES, RUN_MIN_PROB)
    run_ok = True
    run_prob = None
    if run_signal_group is not None:
        run_ok = (run_signal_group["next_value"] == vote_group)
        run_prob = run_signal_group["prob"]
    ...
    bet_group = vote_group if trade else None
    bet_color = vote_color if trade else None
    hit_group = None
    hit_color = None
    state = "WAIT"
...
run_signal_group = get_run_signal(groups, RUN_MIN_LEN, RUN_MAX_LEN, RUN_MIN_SAMPLES, RUN_MIN_PROB)
run_ok = True
run_prob = None
if run_signal_group is not None:
    run_ok = (run_signal_group["next_value"] == vote_group)
...
next_row = {
    ...
    "trade": False,
    "action": next_action,
    "bet_group": vote_group if can_bet else None,
...
# ================= UI =================
st.title("🎯 Hybrid Lock Window + Run Filter + Regime Filter")
...
st.write("Next Group:", vote_group)
st.write("Next Color:", vote_color)
st.write("Group/Color Match:", group_color_ok)
st.write("Run OK:", run_ok)
st.write("Run Prob:", run_prob)
...
st.markdown(
    ...
    NEXT → {next_action} | GROUP {vote_group if vote_group is not None else "-"} | COLOR {vote_color if vote_color is not None else "-"}
    </div>
    """,
...
if group_pause:
    ...
    st.error(f"🔥 BET FULL → GROUP {vote_group} | COLOR {vote_color}")
elif can_bet and next_action == "BET_SMALL":
    st.warning(f"⚠️ BET SMALL → GROUP {vote_group} | COLOR {vote_color}")
else:
    st.info("WAIT")
